...
def getRSquared(xx,yy):
    ...
    for i in range(0,len(xx)):
        rs.append((float(xx[i])**2) + (float(yy[i])**2))
...
def linearBackground(freq,data,bounds):
    ...
    #plt.plot(freq,linear(freq,*pars),label='Linear Background Fit')
    #plt.plot(backgroundFreq,backgroundData,label='Background Fit')
    #return data
...
def importData():
    ...
    return[freq,r,xx,yy,theta,file_path,vrms]

def importDataManual(file_path):
    f = open(file_path)
    n = 0
    vrms = 0
